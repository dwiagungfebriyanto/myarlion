...
class UpdateInventoryLostRequest extends FormRequest
{
    /**
    ...
    {
        if (auth()->user()->can('edit inventory lost')) {
            return true;
        } else {
            return false;
        }
    ...
    public function rules(Request $request): array
    {
        ...
        $inventoryLost  = $this->route('inventory_lost');

        $unit = ($inventoryStock->unit_id === 1)
                ? 'weight'
                : 'amount' ;

        $maxQuantity = $inventoryStock->$unit + $inventoryLost->$unit;

<div class="modal-content">
    ...
    <form id="edit-form" method="post">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel">Update Specification - {{ $specification->code }}</h4>
        </div>
            ...
            <div class="form-group">
                <div class="col-12 edit-validate-input">
                    ...
                    <select class="form-control select2" required name="main_category" id="edit-main_category" disabled>
                        <option value="{{$specification->main_category_id}}">{{$main_category->code . ' - ' . $main_category->main_category_name}}</option>
                    </select>
                </div>
            </div>
            <div class="form-group">
                <div class="col-12 edit-validate-input">
                    <label for="specification_name">Name<span class="text-danger">*</span></label>
                    <input class="form-control" type="text" id="edit-specification_name" name="specification_name"
                        placeholder="Specification Name" value="{{ $specification->specification_name }}">
                </div>
            </div>
        </div>
...
<script>
    ...
    $('#btn-edit-submit').on('click', function(e) {
        ...
        let dataInput = {
            ...
            // code: $('#code').val(),
            specification_name: $('#edit-specification_name').val(),
        };
        $.ajax({
            type: 'PUT',
            url: '{{ route('product.specification.update', $specification) }}',
            data: dataInput,
            success: function(data) {
                // Handle success response
                $('#editModal').modal('hide');
                $('#edit-form')[0].reset();
                $('#specifications-table').DataTable().ajax.reload();
                $.toast({
                    heading: "Success!",
                    text: 'Specification data successfully updated.',
                    position: "top-right",
                    loaderBg: "#5ba035",
                    icon: "success",
                    hideAfter: 3e3,

<!-- sample modal content -->
<div id="viewModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class="modal-title" id="myModalLabel">Detail Vendor - <p class="name" id="name"
                        style="display:inline;">
                    </p>
                </h4>
            </div>
            <div class="modal-body">
                <div class="form-group">
                    <div class="col-12">
                        <label for="address">Address</label>
                        <p class="address" id="address"></p>
                    </div>
                </div>

                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <div class="col-12">
                                <label for="fax">Fax</label>
                                <p class="fax" id="fax"></p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <div class="col-12">
                                <label for="telp">Telp/Hp</label>
                                <p class="telp" id="telp"></p>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="form-group">
                    <div class="col-12">
                        <label for="email">Email</label>
                        <p class="email" id="email"></p>
                    </div>
                </div>

                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <div class="col-12">
                                <label for="contact">Contact</label>
                                <p class="contact" id="contact"></p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <div class="col-12">
                                <label for="pkp">PKP</label>
                                <p class="pkp" id="pkp"></p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-12">
                        <label for="no_rekening">Nomor Rekening</label>
                        <p class="no_rekening" id="no_rekening"></p>
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-12">
                        <label for="note">Note</label>
                        <p class="note" id="note"></p>
                    </div>
                </div>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
